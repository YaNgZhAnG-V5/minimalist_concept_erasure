...
def args_parser():
    ...
    parser.add_argument("--device2", "-d2", type=int, default=2)
    ...
    parser.add_argument("--output-dir", "-o", type=str, default="ca_epoch_10_beta_0_1")
    ...
    for epoch_id in range(args.epochs):
        ...
        for sample_idx, sample in alive_it(enumerate(dataloader), total=len(dataloader)):
            ...
            with torch.no_grad():
                ...
                anchor_noise_pred = pipe.transformer(
                    hidden_states=latents,
                    timestep=timestep / 1000,
                    guidance=None,
                    pooled_projections=pooled_emb_n,
                    encoder_hidden_states=emb_n,
                    txt_ids=text_ids_n,
            ...
            t = timesteps[time_step.item()]
            timestep = t.expand(latents.shape[0]).to(latents.dtype)
            target_noise_pred = target_pipe.transformer(
                hidden_states=latents.to(device2),
                timestep=timestep.to(device2) / 1000,
                guidance=None,
                pooled_projections=pooled_emb_p.to(device2),
                encoder_hidden_states=emb_p.to(device2),
                txt_ids=text_ids_p.to(device2),
                img_ids=latent_image_ids.to(device2),
                # joint_attention_kwargs=pipe.joint_attention_kwargs,
                return_dict=False,
            )[0]
            anchor_noise_pred.requires_grad = False

            # compute loss
            loss = criteria(target_noise_pred, anchor_noise_pred.to(device2))
            loss += beta * criteria(target_noise_pred, noise.to(device2))
        ...
        target_pipe.save_pretrained(f"{args.output_dir}/ca_{train_method}_{epoch_id}")
